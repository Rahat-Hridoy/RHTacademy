...
export const StudentManagement = ({ 
  ...
  const renderTabContent = () => {
    ...
    switch (tab) {
      ...
      case 'Action': return <ActionTab student={selectedStudent} />;
      default: return null;
    }
  };
  ...
  return (
    <>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          {view === 'list' ? (
              ...
              <table className="w-full min-w-[650px] text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {visibleStudents.map(student => (
                    <tr key={student.id} className={`${selectedStudentId === student.id ? 'bg-blue-50/50' : ''} hover:bg-slate-50 transition-colors`}>
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {visibleStudents.map(student => (
                <article key={student.id} className={`rounded-xl border p-4 transition-colors ${selectedStudentId === student.id ? 'border-blue-400 bg-blue-50/30 shadow-sm' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      {selectedStudent && (
        <section aria-labelledby="detail-heading" className="rounded-2xl border border-slate-200 bg-white shadow-sm mt-8 animate-in slide-in-from-bottom-4 fade-in duration-300">
          <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-5 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 id="detail-heading" className="mt-1 text-xl font-bold">{selectedStudent.admin_custom_name || selectedStudent.full_name}</h2>
            </div>
            <div className="flex items-center gap-2">
              <span className={`h-2 w-2 rounded-full ${selectedStudent.account_status === 'paused' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
              <span className={`text-sm font-semibold ${selectedStudent.account_status === 'paused' ? 'text-amber-700' : 'text-emerald-700'}`}>
